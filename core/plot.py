...
def _plot(df, ax, x, y, logx=False, logy=False, k_poly_fit=None, hue=None, style=None, size=None, palette=None):
    if logx:
        _x = np.log(df[x])
    else:
        _x = df[x]
    ...
    sns.scatterplot(
        ...
        edgecolor=(0,0,0,0.8), color=(1,1,1,0), linewidth=1, ax=ax, palette=palette
    )
...
def plot(decon, x, y, logx=False, logy=False, k_poly_fit=None, hue=None, style=None, size=None, path=None, palette=None, fmt="png", figsize=(8,8), dpi=100, separate_ct=False, xlim=None, ylim=None):
    if separate_ct:
        ...
        for i, cell_type in enumerate(decon.cell_types):
            ...
            _plot(decon.sadata.var, ax[i], f"{x}_{cell_type}", f"{y}_{cell_type}", logx=logx, logy=logy, k_poly_fit=k_poly_fit, hue=hue, style=style, size=size, palette=palette)
        ...
        _plot(decon.sadata.var, ax, x, y, logx=logx, logy=logy, k_poly_fit=k_poly_fit, hue=hue, style=style, size=size, palette=palette)
        if xlim != None:
            ax.set_xlim(xlim)
        if ylim != None:
            ax.set_ylim(ylim)
...
def dispersion_plot(
    ...
    size=None, path=None, fmt="png", figsize=(8,8), dpi=100, separate_ct=False, palette=None):

    plot(
        decon, x="mu", y="cv2", logx=True, logy=True, hue=hue,
        style=style, size=size, path=path, fmt=fmt, figsize=figsize,
        dpi=dpi, k_poly_fit=2, separate_ct=separate_ct, palette=palette,
    )


def dropout_plot(
    decon, hue="ct_marker_abs_score_max", style="dropout_outlier",
    size=None, path=None, fmt="png", figsize=(8,8),
    dpi=100, separate_ct=False, xlim=None, ylim=(-0.1, 1.1), palette=None):
...
def gene_weight_hist(weights, xlabel, logy=False, fig_path=None, figsize=(8,8), dpi=80):
    f, ax = plt.subplots(figsize=figsize, dpi=dpi)
...
def marker_plot(decon, top_n_genes=20, show=False, fmt="png", figsize=(8,8), dpi=100):
    ...
    sc.pl.heatmap(decon.sadata, decon.marker_genes[:top_n_genes], swap_axes=True, groupby=decon.label_key, cmap="viridis", dendrogram=True, show=show, save=f".{fmt}" if not show else None)
...
def clustermap(adata, clusterby="leiden", categorical_features="sample", n_genes=10,  cmap="seismic", quantiles=(0.0, 1.0), figsize=(25, 25), dpi=50, fig_path=None, min_cells=100):
    
    # Make barcode into a categorical column so that we can dendrogram by cells
    adata.obs["barcode"] = pd.Categorical(adata.obs.index)

    palettes = [sns.color_palette("tab10"), sns.color_palette("Set2"), sns.color_palette("Paired")]
    _grid = rcParams["axes.grid"]
    rcParams["axes.grid"] = False
    ...
    for i, group_i in enumerate(groups):
        ...
        for j, group_j in enumerate(groups):
            data = adata[dendro_order, gene_groups[j,:]].layers["logcentered"].T.toarray()
            sns.heatmap(
                data,
                ax=axes[j+len(categorical_features)][i+1], cmap=cmap, center=0, vmin=vmin, vmax=vmax, cbar=False, yticklabels=gene_groups[j,:]
            )
            axes[j+len(categorical_features)][i+1].patch.set_edgecolor("black")
            axes[j+len(categorical_features)][i+1].patch.set_linewidth(2.0)
        
    f.colorbar(plt.cm.ScalarMappable(norm=matplotlib.colors.TwoSlopeNorm(vmin=vmin, vmax=vmax, vcenter=0), cmap=cmap), ax=axes, orientation="vertical", fraction=0.05, pad=0.05, shrink=0.2)
...
def pseudo_bulk_plot(decon, hue="log_bulk_residual", style=None, dir=None, fmt="png", figsize=(8,8), dpi=80, palette="seismic", vmin=None, vmax=None, vcenter=None):
    if vmin == None:
    ...
    for i in range(decon.badata.shape[0]):
        ...
        sns.scatterplot(
            data=decon.sadata.var, x="pseudo", y=f"bulk_{i}", c=decon.sadata.var[hue] if hue else None, style=style,
            edgecolor=(0,0,0,1), ax=ax, linewidth=1, cmap=colormap, norm=normalize
        ).set_title(f"Sample {i}")

        ax.plot([0, decon.sadata.var[f"bulk_{i}"].max()], [0, decon.sadata.var[f"bulk_{i}"].max()], label="y=x", c="royalblue")
