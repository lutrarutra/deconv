...
class LogNormalRefModel(ReferenceModel):
    def __init__(self, adata: sc.AnnData, labels_key: str, dropout_type: Literal["separate", "shared", None], device: Literal["cpu", "cuda"], layer: str | None, fp_hack: bool):
        ...
        self.dec_model_cls = LogNormalDecModel
    ...
    def plot_pdf(self, adata: sc.AnnData, cell_type: str, gene: str, ax: plt.Axes, n_samples: int = 5000):
        ...
        mu = self._params["mu"][gene_idx, cell_type_idx]
        sigma = self._params["sigma"][gene_idx, cell_type_idx]
        theta = dist.LogNormal(mu, sigma).sample(torch.Size((n_samples,)))
...
class LogNormalDecModel(DeconvolutionModel):
    def model(self):
