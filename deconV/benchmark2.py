...
def main(args):
    ...
    with open("results_xin.txt", "w") as f:
    ...
    for values in pbar:
        ...
        rmse = dv.pl.rmse(est_df, true_df)

        with open("results_xin.txt", "a") as f:
            f.write(f"{rmse:.5f}\t{' '.join([f'{v:.3f}' for v in est_df.values.flatten()])}\t{str(params)}\n")
